...
export const ChapterMapScreen: React.FC = () => {
  ...
  const { currentChapterId, currentNodeId, visitedNodes, day, timeBlock, elapsedHours } = store;
  ...
  if (!chapter) {
    return <main className="screen-container screen-center"><p className="muted-copy glitch-text" data-text="지도 자료가 손상되었습니다.">지도 자료가 손상되었습니다.</p></main>;
  }
  ...
  const getNodeType = (nodeId: string) => {
    ...
    if (isRestEligibleNode(chapter.chapter_id, node)) return "safehouse";
    const events = chapter.events.filter((event) => event.node_id === nodeId);
    const types = events.map((event) => String(event.event_type).toLowerCase());
    if (types.some((type) => type === "battle" || type === "boss")) return "combat";
    if (types.some((type) => type === "loot" || type === "search" || type === "exploration")) return "search";
    return "story";
  };

  return (
    <main className="screen-container map-screen" style={{ backgroundImage: bgImage ? `linear-gradient(rgba(5,7,8,0.85), rgba(5,7,8,0.95)), url('${bgImage}')` : undefined }}>
      ...
      <section className="map-layout" style={{ position: "relative", zIndex: 10 }}>
        <aside className="map-info glass-panel tactical-frame" style={{ border: "none", background: "rgba(5, 7, 8, 0.75)" }}>
          <header>
            <p className="eyebrow" data-text="현장 시야">현장 시야</p>
            <h1 style={{ fontFamily: "var(--heading-font)", textTransform: "none" }}>{currentNode?.name ?? "작전 시작 지점"}</h1>
          </header>
          <div style={{ marginTop: "12px", borderTop: "1px solid rgba(156,207,214,0.1)", paddingTop: "16px" }}>
            <p style={{ fontSize: "0.95rem", lineHeight: "1.6", color: "var(--text-main)" }}>{currentNode?.description ?? "진입 가능한 구역을 선택해 다음 작전 목표로 이동합니다."}</p>
          </div>
          ...
          <div style={{ marginTop: "auto", paddingTop: "24px", fontSize: "0.7rem", fontFamily: "var(--mono-family)", opacity: 0.5 }}>
            <div>지도 배율 0.84</div>
            <div>신호 강도 92%</div>
          </div>
        </aside>
        ...
        <div className="map-board glass-panel tactical-frame" style={{ border: "none", background: "rgba(0, 0, 0, 0.3)" }}>
          ...
          <svg className="map-lines" viewBox="0 0 100 100" preserveAspectRatio="none" aria-hidden="true" style={{ opacity: 0.6 }}>
            {chapter.nodes.flatMap((node) => node.connections.map((connection) => {
              ...
              return <line key={`${node.node_id}-${connection.to}`} x1={node.coordinates.x} y1={node.coordinates.y} x2={target.coordinates.x} y2={target.coordinates.y} style={{ stroke: active ? "var(--accent-color)" : visited ? "rgba(156, 207, 214, 0.4)" : "rgba(255,255,255,0.08)", strokeWidth: active ? 0.8 : 0.4, strokeDasharray: active ? "none" : "1, 2" }} />;
            }))}
          </svg>
