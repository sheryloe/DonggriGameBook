...
import { getChapterRuntimeConfig } from "@donggrol/world-registry";
...
export function resolveSpecialScreenId(
  eventId: string | null,
  uiFlow: UIFlow | undefined,
  expectedScreenType?: UIScreenType
): string | null {
  if (!uiFlow || !eventId) {
    return null;
  }
  ...
  if (expectedScreenType && targetScreen.screen_type !== expectedScreenType) {
    return null;
  }

  return targetScreen.screen_id;
}
