...
export function getEndingCardById(
  ...
  for (const chapterIdInPack of content.chapter_order) {
    ...
    return {
      ...
      summary: matchedRule.summary || `${humanizeEndingId(endingId)} route result.`,
      hint: matchedRule.hint || `${humanizeEndingId(endingId)} unlock conditions are not yet satisfied.`,
  ...
  return {
    ...
    summary: `${humanizeEndingId(endingId)} ending reached.`,
    hint: `${humanizeEndingId(endingId)} remains locked.`,
    art_key: buildEndingArtKey(endingId),
    thumb_key: buildEndingThumbKey(endingId),
    video_id: buildEndingVideoId(endingId),
    unlocked_at: unlockedAt,
  ...
  for (const chapterId of content.chapter_order) {
    ...
    for (const rule of sortedRules) {
      ...
      cards.set(rule.ending_id, {
        ...
        summary: rule.summary || `${humanizeEndingId(rule.ending_id)} ending reached.`,
        hint: rule.hint || `${humanizeEndingId(rule.ending_id)} remains locked.`,
